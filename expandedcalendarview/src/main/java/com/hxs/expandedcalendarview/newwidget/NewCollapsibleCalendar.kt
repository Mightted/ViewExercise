package com.hxs.expandedcalendarview.newwidget
...
class NewCollapsibleCalendar : NewUICalendar {

    private var dateFormat = SimpleDateFormat("yyyy-MM-dd", getCurrentLocale(context))
    private var isCurFragment = true

    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : super(context, attrs, defStyleAttr)
    constructor(context: Context) : super(context)
    constructor(context: Context, attrs: AttributeSet?) : super(context, attrs)

    override fun changeToToday(calendar: Calendar) {
...
//    }
    ...
    val suitableRowIndex: Int
        get() {
            if (selectedItemPosition != -1) {
                val view = mAdapter?.getView(selectedItemPosition)
                val row = view?.parent as TableRow? ?: return 0
...
//    }
    ...
    fun init(context: Context, cal: Calendar) {

//        if (force || mAdapter == null || mAdapter?.calendar != cal) {
//
//        }
...
//        }
    ...
    override fun reload() {
        mAdapter?.let { mAdapter ->
            mAdapter.refresh(weekOffset, state == STATE_EXPANDED)
            weekOffset = 0
//            val calendar = Calendar.getInstance()
...
//                if (state == STATE_COLLAPSED) {
//                    if (mCurrentWeekIndex == -1 && i != mAdapter.count - 1) {
//                        continue
//                    } else if (i < mCurrentWeekIndex) {
//                        continue
//                    }
//                }
                ...
                params.let { params ->
                    if (params != null && (mAdapter.getItem(i).diff < params.prevDays || mAdapter.getItem(i).diff > params.nextDaysBlocked)) {
                        ...
                        if (view.isClickable) {
                            view.setOnClickListener { v -> onItemClicked(mAdapter.getItem(i)) }
                        }
                    }
                }
                rowCurrent.addView(view)
...
//        }
    ...
    fun setStateWithUpdateUI(state: Int) {
        this@NewCollapsibleCalendar.state = state
