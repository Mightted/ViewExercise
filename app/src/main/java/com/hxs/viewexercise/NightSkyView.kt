 ...
 * Description:
 */
class NightSkyView : AppCompatImageView {
    ...
    fun animator() {
        post {
            meteors.add(Meteor(floatArrayOf(width - 200f, 200f), floatArrayOf(200f, 0.4f * height)))
            meteors.add(Meteor(width.toFloat(), height * 0.5f))
...
//        private var lineEnd = FloatArray(2)
        ...
        private fun updatePath(start: FloatArray, end: FloatArray) {
            ...
            path.quadTo(
                (end[0] + start[0]) / 2 - Random.nextInt(50),
                (end[1] + start[1]) / 2 - Random.nextInt(50),
                end[0],
                end[1]
            )
        ...
        private fun updateAlpha(): Int {
            ...
            val progress = when {
                ...
                pathMeasure.length - translate <= tailLength -> {
                    (pathMeasure.length - translate) / tailLength
                }
                else -> {
                    1f
...
//            textPaint.color = ColorUtils.setAlphaComponent(textPaint.color, updateAlpha())
//                drawTextOnPath(
//                    "✨",
//                    movePath,
//                    textPaint.textSize,
...
//                drawText("✨", start[0], start[1], textPaint)
